...
from cari.composition import ApplicationContext, build_application_context
...
from cari.database.startup import DatabaseStartupError, prepare_database
...
class ApplicationStartupError(Exception):
    """Raised when Cari cannot prepare its local application infrastructure."""
...
def create_application_context(
    ...
    try:
        ...
        raise ApplicationStartupError(
            "Cari could not create or access its application data directory."
        ) from error
    ...
    try:
        ...
        raise ApplicationStartupError("Cari could not open its local database safely.") from error
...
def main() -> int:
    ...
    try:
        ...
        LOGGER.error("Cari startup failed: %s", error)
        QMessageBox.critical(None, "Cari Startup Error", str(error))
        return 1
    except Exception:
        LOGGER.exception("Unexpected error during Cari startup")
        QMessageBox.critical(
            None,
            "Cari Startup Error",
            "Cari could not start safely. No existing database was replaced.",
        )
        return 1
