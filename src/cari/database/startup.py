...
class InvalidDatabaseError(DatabaseStartupError):
    """Raised when an existing file is not a valid Cari database."""
...
def get_migration_head() -> str:
    """Return the single migration head bundled with this Cari version."""
    config = _create_script_config()
    heads = ScriptDirectory.from_config(config).get_heads()
    if len(heads) != 1:
        raise DatabaseStartupError("Cari requires exactly one Alembic migration head.")
    return heads[0]
...
def inspect_database(database_path: Path) -> DatabaseStatus:
    ...
    if current_revision == head_revision:
        ...
        if table_names != expected_tables:
            return DatabaseStatus(
                ...
                head_revision,
                current_revision,
                "The database tables do not match the current Cari schema.",
            )
        if not _schema_matches_metadata(resolved_path):
            return DatabaseStatus(
                ...
                head_revision,
                current_revision,
                "The database schema does not match the current Cari migration.",
            )
    ...
    if not _is_known_ancestor(current_revision, head_revision):
        return DatabaseStatus(
            ...
            head_revision,
            current_revision,
            "The database revision is not recognized by this Cari version.",
        )
...
def prepare_database(database_path: Path) -> DatabaseStatus:
    ...
    if status.state is DatabaseState.OUTDATED:
        raise DatabaseOutdatedError(
            "The Cari database requires migration, but automatic upgrades are disabled "
            "until recovery backups are implemented."
        )
    raise InvalidDatabaseError(
        "The existing database is not a valid current Cari database and was left unchanged."
    )
...
def initialize_database_to_head(database_path: Path) -> DatabaseStatus:
    ...
    try:
        ...
        raise DatabaseInitializationError(
            "Cari could not initialize the new database; the final path was left untouched."
        ) from error
    finally:
